...
int main()
{
    ...
    这种别名的作用：
    1、用作函数的形参，通过将引用变量用作参数，函数将使用原始数据，而不是副本。
    2、除指针之外，引用也为函数处理大型结构提供了一种方便途径。
    3、引用在设计类时也是必不可少的
    */
    ...
    /*声明引用的同时必须初始化，不能像普通指针一样可以先声明再初始化（注意const指针必须在创建时进行初始化）
      引用变量近似与指针常量：int &rodents=rats; == int *const pr=&rats;
      
      引用变量一旦和某变量关联就不能再和其他变量关联了，一旦重关联，虽然语法上不会报错，但是会改变引用变量被引
      用的变量的值*/
    int bunnies = 50;
    rodents = bunnies;  //试图改变rodents的引用
    cout << "bunnies = " << bunnies;
    cout << ", rats = " << rats;/*rats会从原先的101变为bunnies的值50,相当于执行了rats=bunnies*/
    /*可见，对引用变量施加的影响同时会作用在被引用变量上*/
    cout << ", rodents = " << rodents << endl;
